...
module TypeTracking<TypeTrackingInput I> {
  ...
  module TypeTrack<endpoint/1 source> {
    ...
    module Graph<endpoint/1 sink> {
      private newtype TPathNode = TPathNodeMid(Node node, TypeTracker tt) { node = flow(tt) }
      or TPathNodeSink(Node node) { sink(node) and flowsTo(node) }
      ...
      private predicate edgeCand(PathNodeFwd n1, PathNodeFwd n2) {
        ...
        |
          n2 = tgt or n2 = TPathNodeSink(tgt.getNode()) and tgt.getTypeTracker().end()
          )
        or
